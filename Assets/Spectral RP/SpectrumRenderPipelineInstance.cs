    ...
    public class SpectrumRenderPipelineInstance : RenderPipeline
    {
        ...
        protected override void Render(ScriptableRenderContext context, List<Camera> cameras)
        {
            ...
            foreach (Camera camera in cameras)
            {
                ...
                SetupLights(context, ref cullingResults);
        ...
        public void SetupLights(ScriptableRenderContext context, ref CullingResults cullResults)
        {
            for (int i = 0; i < MaxLightCount; i++)
            {
                ...
                {
                    _lightData[i] = light.localToWorldMatrix.MultiplyVector(Vector3.back);
                    _lightColor[i] = light.finalColor;
                    _lightColor[i].w = -1; //for identifying it is a directional light in shader
                }
                else if (light.lightType == LightType.Point)
                {
                    _lightData[i] = light.localToWorldMatrix.GetPosition();
                    _lightData[i].w = light.range;
                    _lightColor[i] = light.finalColor;
                    _lightColor[i].w = -2; //for identifying it is a point light in shader
                }
                else if (light.lightType == LightType.Spot)
                {
                    _lightData[i] = light.localToWorldMatrix.GetColumn(3);
                    _lightData[i].w = 1f / Mathf.Max(light.range * light.range, 0.00001f);

                    _lightSpotDir[i] = light.localToWorldMatrix.GetColumn(2);
