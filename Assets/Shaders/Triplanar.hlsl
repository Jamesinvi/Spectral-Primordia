...
real3 TriplanarNormalMap(real3 posWS, real3 normWS, sampler2D tex, real normalStrength, real triplanarScale, real blendSharpness, real scrollSpeed)
    {
        ...
        float3 nAx = UnpackNormalScale(tex2D(tex, triUVA.zy), normalStrength);
        float3 nAy = UnpackNormalScale(tex2D(tex, triUVA.xz), normalStrength);
        float3 nAz = UnpackNormalScale(tex2D(tex, triUVA.xy), normalStrength);
